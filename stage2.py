        ...
        with torch.no_grad():
            ...
            for i in generated_tokens:
                ...
                if "t5" in args.model_name:
                    ...
                    if args.dataset in ["GSM8K","CSQA","AQuA","SecQA","CyberNERQA"]:
...
def stage2(model_name, data_path="./data/", dataset="CyberNERQA", out_path="output/big2small/", 
           batch_size=5, max_gen_len=100, 
           stage1_args, guidances, #instead of big_output_path, pass directly instead of reading from a file
           few_shot=1)
    ...
    # stage1_len=stage1_args['max_gen_len'] #args.big_output_path.split("/")[-2] 
    base_name = "output_stage2_fs"+{few_shot}+"_msk"+{stage1_args["masking"]}
    out_path=os.path.join(folder_name,basename,".jsonlines")
    ...
    basej_name = "args_stage2_fs"+{few_shot}+"_msk"+{stage1_args["masking"]}
    ...
    if args.dataset =="SecQA":
        dataset=SecQA_dataset(tokenizer,args,stage2=True)
    elif args.dataset =="CyberNERQA":
        dataset=CyberNERQA_dataset(tokenizer,args,stage2=True)
